...
export default function AttendanceInForm({ userId, onSubmitted, onCancel }: AttendanceInFormProps) {
  ...
  useEffect(() => {
    navigator.mediaDevices.getUserMedia({ video: true })
      .then(() => setStep('camera'))
      .catch(() => {
        toast.error("Permission Required", { description: "Camera access is needed to check in." });
        onCancel();
      });
  }, [onCancel]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Step 2: Submit the attendance data with all fields from your schema
      const attendancePayload = {
        ...
        inTimeImageUrl: publicUrl,
        inTimeLatitude: location.coords.latitude,
        inTimeLongitude: location.coords.longitude,
        inTimeAccuracy: location.coords.accuracy,
        inTimeSpeed: location.coords.speed,
        inTimeHeading: location.coords.heading,
        inTimeAltitude: location.coords.altitude,
      };

      // Debugging: Log the payload to the console
      //console.log("Submitting this payload to /api/attendance/check-in:", JSON.stringify(attendancePayload, null, 2));

      const attendanceResponse = await fetch(`${BASE_URL}/api/attendance/check-in`, {
        method: 'POST',
      ...
      if (!attendanceResponse.ok) {
        const result = await attendanceResponse.json().catch(() => ({ error: 'Check-in submission failed.' }));
      ...
      case 'camera':
        return (
          <>
            ...
            <p className="text-muted-foreground text-center mb-6">Please take a selfie to mark your attendance.</p>
      ...
      case 'confirm':
        return (
          <>
            ...
            <p className="text-muted-foreground text-center mb-6">Review your photo and location before checking in.</p>
