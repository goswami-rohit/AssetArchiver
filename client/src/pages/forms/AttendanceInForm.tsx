...
export default function AttendanceInForm({ userId, onSubmitted, onCancel }: AttendanceInFormProps) {
  ...
  const fetchLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        setStep('camera'); // Revert to camera step if location fails
      }
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    formData.append('inTimeImage', await dataURLtoBlob(photoUri), 'checkin.jpg');
    ...
    try {
      ...
      if (!response.ok) {
        const result = await response.json().catch(() => ({ error: 'Failed to check in.' }));
      ...
      case 'camera':
        return (
          <>
            ...
            <p className="text-muted-foreground text-center mb-6">Please take a clear selfie to mark your attendance.</p>
      ...
      case 'confirm':
        return (
          <>
            ...
            <p className="text-muted-foreground text-center mb-6">Review your photo and location before submitting.</p>
